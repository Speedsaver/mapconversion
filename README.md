...
*Note our Speedsaver repos have set the filesystem size at a fixed 700 MB. Depending on your map chosen and the method of conversion, you may encounter a fatal “max_leb_cnt to low” error when compiling. Simply run (in a tty virtual terminal, NOT in the GUI. Tty virtual console is usually accessed by pressing Ctrl+Alt+F2 or Ctrl+Alt+F3)

```console
make menuconfig
```

Navigate to "Filesystem images" / exact size and change 700M to something that will accomodate your map/s, e.g. 3000M for OSM UK and Ireland. Also navigate to "Logical eraseblock size", convert the hex value here to decimal (Google it), double that number, convert the answer back to hex, insert this value then save and exit. Again run:
